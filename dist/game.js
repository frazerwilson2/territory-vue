...
var game = new Vue({
  ...
  methods: {
    ...
    wChampVoteNo: function wChampVoteNo() {
    ...
    voteThis: function voteThis(vote, index) {
      ...
      var voteCount = 0;
      var sum = game.voteCard.reduce(function (acc, val) {
    ...
    voteApproved: function voteApproved() {
          ...
          switch (player.news.enact.type) {
            ...
            case 'starInc':
              ...
              player.news.enact.starInc.forEach(function (w) {
                ...
                if (game.findWrestler(w).inc > 3) {
                  game.findWrestler(w).inc = 3;
                };
    ...
    resetVoteCard: function resetVoteCard() {
    ...
    isWChampAble: function isWChampAble(w) {
      var prospect = this.findWrestler(w);
      if (prospect.type !== 'singles') {
        return false;
      }
      if (prospect.val + prospect.inc < 8) {
        return false;
      }
      return true;
    },
    ...
    switchHeel: function switchHeel(player, match) {
    ...
    gimmickBonus: function gimmickBonus(bonus, compets) {
      var _this = this;

      var result = false;
      compets.forEach(function (w) {
        var wrestle = _this.findWrestler(w);
    ...
    checkArena: function checkArena(value, index) {},
    ...
    awardTopDraw: function awardTopDraw() {
      ...
      var highestVal = Math.max.apply(Math, drawTotals);
      var numWinners = this.countTotal(drawTotals, highestVal);
      if (numWinners > 1) {
    ...
    addToCard: function addToCard(wrestler, id, first, firstIndex) {
      if (first) {
    ...
    addGimmickCard: function addGimmickCard(gimmick, index) {
      ...
      this.closePop();
      if (this.findGimmick(gimmick).affect == 'true') {
    ...
    resetMatch: function resetMatch(player, match) {
      console.log(player, match);
      player.matchcard[match].competitors.forEach(function (w) {
        console.log(w);
    ...
    validateMatch: function validateMatch(match, titleswitch) {
    ...
    textReq: function textReq(r) {
      var split = r.split(':');
      var msg = 'guy with ';
      msg += split[0];
      if (split[1][0] == '>') {
        msg += ' more than ' + split[1].substring(1);
      }
      if (split[1][0] == '<') {
        msg += ' less than ' + split[1].substring(1);
      }
      if (split[1][0] == '=') {
    ...
    enactSwap: function enactSwap(i) {
      var complete = false;
      var swapRecord = this.tradeProposeList[i];
